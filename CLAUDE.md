...
- `src/sockets/SocketManager.ts` — singleton `socketManager` wrapping a raw `WebSocket` to `env.wsUrl`, with automatic exponential-backoff reconnect (1s base, 30s cap), a staleness watchdog that drops a connection which stops delivering frames (a half-open socket never fires `close` on its own), and pub/sub via `onMessage`/`onStatusChange`, each returning an unsubscribe function. `FleetProvider` is the app's **only** subscriber — it owns the connection lifecycle (`connect` on mount, `disconnect` on unmount) and republishes what components need as `connectionStatus` on the fleet context. Read connection state via `useFleet()`; don't subscribe to `socketManager` from a component.
